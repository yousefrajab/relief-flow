<x-app-layout>
    <div class="space-y-8">
        <x-welcome-banner :title="__('Welcome back, :name', ['name' => auth()->user()->name])" :subtitle="__('Full visibility across warehouses, items, requests, and accounts.')" />

        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <a href="{{ route('warehouses.index') }}" class="bg-white border border-ink-100 rounded-2xl p-5 hover:border-field-300 hover:shadow-md transition-all">
                <div class="w-9 h-9 rounded-xl bg-field-50 text-field-600 flex items-center justify-center mb-3"><x-icon name="warehouse" class="w-4.5 h-4.5" /></div>
            ...
            <a href="{{ route('items.index') }}" class="bg-white border border-ink-100 rounded-2xl p-5 hover:border-field-300 hover:shadow-md transition-all">
